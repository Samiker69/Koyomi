...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (amount < 1 || amount > 100) {
            return interaction.reply({ content: 'Укажи количество сообщений от 1 до 100.', ephemeral: true });
        }
        ...
        try {
            ...
            if (userMessages.size === 0) {
                return interaction.reply({ content: 'Нет сообщений для удаления.', ephemeral: true });
            }

            await channel.bulkDelete(userMessages, true);

            const reply = await interaction.reply({ content: `Удалено ${userMessages.size} сообщений.`, ephemeral: true });
