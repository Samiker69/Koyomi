...
module.exports = {
	...
	if (timestamps.has(interaction.user.id)) {
		...
		if (now < expirationTime) {
			...
			return interaction.reply({ content: `Не так быстро! вы слишком часто использовали \`${command.data.name}\`. Ты снова сможешь использовать её <t:${expiredTimestamp}:R>.`, flags: MessageFlags.Ephemeral });
		}
	}
